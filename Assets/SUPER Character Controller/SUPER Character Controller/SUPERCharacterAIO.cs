...
namespace SUPERCharacter
{
...
#if ENABLE_INPUT_SYSTEM
        public Key sprintKey = Key.LeftShift, crouchKey = Key.LeftCtrl, slideKey = Key.V, lockCameraKey = Key.L;
#else
    ...
    public KeyCode interactKey_L = KeyCode.E;
        ...
        void Update()
        {
            if (Keyboard.current[lockCameraKey].isPressed)
            {
                if (timer <= 0)
                {
                    timer = 0.5f;
                    controllerPaused = !controllerPaused;
                    Cursor.lockState = controllerPaused ? CursorLockMode.None : CursorLockMode.Locked;
                    Cursor.visible = controllerPaused;
                }
            }
